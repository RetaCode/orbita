...
const PriorityBadge = ({ priority }: { priority: 'baja' | 'media' | 'alta' | 'critica' }) => (
  <span
...
export function TasksSummary() {
  const todayTasks = tasksData.slice(0, 3);
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Tareas para Hoy</CardTitle>
          <CardDescription>Tienes {tasksData.filter(t => t.estado !== 'completada').length} tareas pendientes.</CardDescription>
        </div>
